...
def initialize_database():
    ...
    with get_db_connection() as conn:
        ...
        """)
        ensure_column(cursor, "alarms", "volume", "INTEGER DEFAULT 85")
...
def add_alarm(time, label="", repeat_days="[]", is_active=1, custom_sound=None, volume=85):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO alarms (time, label, repeat_days, is_active, snooze_count, custom_sound, volume) VALUES (?, ?, ?, ?, 0, ?, ?)",
            (time, label, repeat_days, is_active, custom_sound, int(volume))
        )
...
def update_alarm(alarm_id, time=None, label=None, repeat_days=None, is_active=None, custom_sound=None, volume=None):
    fields = []
    values = []
    if time is not None:
        fields.append("time = ?")
